#ifndef CookTorranceFInv_hpp
#define CookTorranceFInv_hpp

#include "BRDF.hpp"
#include <cmath>
#include <algorithm>

// Cook-Torrance com Fresnel invertido: F = F0 + (1-F0)*cosTheta^5.
// Comportamento oposto ao fisico: maxima reflexao no centro (incidencia normal),
// minima reflexao nas bordas (incidencia rasante). Completamente nao fisico.
class CookTorranceFInv : public BRDF {
public:
    float roughness;
    float metallic;

    CookTorranceFInv() : roughness(0.5f), metallic(0.0f) {}
    ...
    RGB f(Vector wi, Vector wo, Vector N, const BRDF_TYPES type = BRDF_ALL) override {
        ...
        if (NdotL > 0.f && NdotV > 0.f) {
            ...
            if (hlen > 1e-6f) {
                ...
                // invertido: VdotH^5 em vez de (1-VdotH)^5
                // VdotH=1 (normal) -> f=1 -> F=1 (max); VdotH=0 (rasante) -> f=0 -> F=F0 (min)
                float f_schlick = powf(VdotH, 5.f);
