#ifndef CookTorranceGNone_hpp
#define CookTorranceGNone_hpp

#include "BRDF.hpp"
#include <cmath>
#include <algorithm>

// Cook-Torrance sem termo G: G = 1 (sem correcao de auto-sombra/masking).
// Standard usa G = G1L * G1V (Smith) que atenua o especular em angulos rasantes
// e superficies rugosas. Sem G, o highlight nao e atenuado — brilho excessivo
// em materiais rugosos, especialmente visivel a angulos obliquos.
class CookTorranceGNone : public BRDF {
public:
    float roughness;
    float metallic;

    CookTorranceGNone() : roughness(0.5f), metallic(0.0f) {}
    ...
    RGB f(Vector wi, Vector wo, Vector N, const BRDF_TYPES type = BRDF_ALL) override {
        ...
        if (valid_H && (type & GLOSSY_REF)) {
            ...
            // G = 1: sem correcao de shadowing-masking
            color += F * (D / (4.f * NdotL * NdotV));
        }
