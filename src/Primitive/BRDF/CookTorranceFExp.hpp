#ifndef CookTorranceFExp_hpp
#define CookTorranceFExp_hpp

#include "BRDF.hpp"
#include <cmath>
#include <algorithm>

// Cook-Torrance com Fresnel de expoente exagerado: (1-cosTheta)^20 em vez de ^5.
// A transicao para reflexao total nas bordas e muito mais abrupta — a borda
// "acende" apenas nos ultimos graus de incidencia rasante.
class CookTorranceFExp : public BRDF {
public:
    float roughness;
    float metallic;

    CookTorranceFExp() : roughness(0.5f), metallic(0.0f) {}
    ...
    RGB f(Vector wi, Vector wo, Vector N, const BRDF_TYPES type = BRDF_ALL) override {
        ...
        if (NdotL > 0.f && NdotV > 0.f) {
            ...
            if (hlen > 1e-6f) {
                ...
                // expoente 20: borda muito mais abrupta que o fisico (^5)
                float f_schlick = powf(1.f - VdotH, 20.f);
