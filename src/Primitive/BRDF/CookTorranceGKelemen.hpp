#ifndef CookTorranceGKelemen_hpp
#define CookTorranceGKelemen_hpp

#include "BRDF.hpp"
#include <cmath>
#include <algorithm>

// Cook-Torrance com aproximacao de G de Kelemen (2001).
// Standard: especular = F * D * G1L*G1V / (4*NdotL*NdotV)
// Kelemen:  G/denominator = NdotL*NdotV / VdotH^2
//           => especular = F * D * NdotL*NdotV / (VdotH^2 * 4*NdotL*NdotV)
//                        = F * D / (4 * VdotH^2)
// NdotL e NdotV cancelam-se — o denominador passa a depender so de VdotH.
class CookTorranceGKelemen : public BRDF {
public:
    float roughness;
    float metallic;

    CookTorranceGKelemen() : roughness(0.5f), metallic(0.0f) {}
    ...
    RGB f(Vector wi, Vector wo, Vector N, const BRDF_TYPES type = BRDF_ALL) override {
        ...
        if (valid_H && (type & GLOSSY_REF)) {
            ...
            // Kelemen: F * D / (4 * VdotH^2)
            float vdh2 = std::max(VdotH * VdotH, 1e-6f);
            color += F * (D / (4.f * vdh2));
        }
...
#endif /* CookTorranceGKelemen_hpp */
