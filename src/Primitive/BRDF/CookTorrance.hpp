...
public:
    ...
    RGB f(Vector wi, Vector wo, Vector N, const BRDF_TYPES type = BRDF_ALL) override {
        ...
        // --- Half-vector e Fresnel Schlick (VdotH) ---
        // F calculado uma vez, partilhado pelo difuso (kD) e pelo especular.
        // Assim a partição de energia difuso+especular é consistente por direção de luz.
        Vector H = wi + wo;
        float hlen = H.norm();
        ...
        // --- Difuso: metais não têm difuso (1-metallic); Fresnel determina
        ...
        if (!Ks_brdf.isZero() && (type & GLOSSY_REF) && NdotL > 0.f) {
