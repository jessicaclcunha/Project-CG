#ifndef CookTorranceFConst_hpp
#define CookTorranceFConst_hpp

#include "BRDF.hpp"
#include <cmath>
#include <algorithm>

// Cook-Torrance com Fresnel constante: F = F0 sempre (sem variacao angular).
// Sem o fator (1-cosTheta)^5, o highlight nao escurece no centro nem clareia nas bordas.
class CookTorranceFConst : public BRDF {
public:
    float roughness;
    float metallic;

    CookTorranceFConst() : roughness(0.5f), metallic(0.0f) {}
    ...
    RGB f(Vector wi, Vector wo, Vector N, const BRDF_TYPES type = BRDF_ALL) override {
        ...
        // F = F0 sempre — sem variacao angular (Schlick removido)
        RGB F = F0;
        float NdotH = 0.f, alpha = roughness * roughness;
        bool valid_H = false;
